-- Complete deployment script for all game functions
-- Run this in your Supabase Dashboard SQL Editor

-- First, ensure all tables exist (from complete_setup.sql)
-- This is a condensed version focusing on the essential functions

-- Create the essential RPC functions for game flow
...
BEGIN
  ...
  -- Bonus point if imposter guesses correctly
  IF p_imposter_guess_index = v_secret_word_index THEN
    v_imposter_score := v_imposter_score + 1;
  END IF;
...
SELECT 
    ...
    (SELECT COUNT(*) FROM pg_proc WHERE proname LIKE '%room%' OR proname LIKE '%clue%' OR proname LIKE '%vote%') as function_count;
